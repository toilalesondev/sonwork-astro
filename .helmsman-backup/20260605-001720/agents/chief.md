...
tier: light
evolver_tier: light
skills: [office-hours, document-generate, handoff, watzup, scrape, benchmark, benchmark-models]
aliases: []
tools:
  write: true
  edit: true
...
- `benchmark` — measure + compare performance numbers. `benchmark-models` — compare models on a task.
- `watzup` — read-only snapshot of everything in flight (process/, git, gbrain).
