...
### EXPLORE — `@scout` (+ `@helm` for cross-repo)
Read-only fact gathering. **Memory first:** `@scout` connects to memory before reading
code (local `process/context/` → gbrain `recall`) so it learns the *delta*, not the whole
repo again — then explores, then **refreshes the learning scaffolding** (local context +
durable gbrain write-back) so each pass starts smarter. `@scout` for this repo's
architecture; `@helm` for cross-repo knowledge + the gbrain code graph (curation). Output: what
exists, what's affected, what's at risk. (First-touch SETUP — install harness, register
the gbrain source, write per-project `AGENTS.md` — is `@helm`'s `onboard-project`; @scout
owns the *learning* half.)
...
(document lessons → return to PLAN), and **self-review** before hand-off. On long builds the
50% check-in also **re-anchors** the active obligations (`adherence.md`) — following the
harness decays over a long trajectory. If stuck, `@ship` (investigate/diagnose) finds root
cause and hands the fix boundary back.
...
### WRITE-BACK — the agent that did the work (Loop A)
Proposes a numbered list of gbrain writes (pages / facts / takes) + `process/`
archival + backlog capture.

**▶ HUMAN GATE:** you approve the numbered list. Then that agent writes gbrain and
...
- **Question / understanding:** routes to `@scout` (or `@helm` for cross-repo), or answered directly if
  trivial conceptual.
