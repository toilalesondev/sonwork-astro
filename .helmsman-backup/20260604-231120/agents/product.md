...
tier: standard
evolver_tier: light
skills: [office-hours, design-shotgun, design-consultation, to-prd, plan-ceo-review, autoplan]
aliases: [gstack]
tools:
  write: true
  edit: true
...
# @product writes concepts/PRDs (edit) but runs NO shell (bash denied).
permission:
  edit: allow
  bash: deny
  read: allow
...
- `plan-ceo-review` — founder-lens scope review: expand / hold / reduce, find the 10-star product
- `autoplan` — run all four plan-*-review skills sequentially, auto-decide, gate the taste calls
