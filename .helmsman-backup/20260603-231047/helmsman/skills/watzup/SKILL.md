...
## Validate (read-only self-check)

watzup must never write. Prove the contract held:
```
# BEFORE producing the snapshot:
BASE=$(node .helmsman/skills/watzup/scripts/validate-watzup.mjs --baseline)
# ... produce the snapshot (read-only) ...
# AFTER:
node .helmsman/skills/watzup/scripts/validate-watzup.mjs --check "$BASE"
```
If the working tree changed, watzup violated its read-only contract — that's a bug to fix.
