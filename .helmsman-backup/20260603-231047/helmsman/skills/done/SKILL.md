...
name: done
version: 1.0.0
description: End-of-session ritual for a shared cross-machine brain. Chains write-back (capture) and brain-repo sync — the agent that did the work proposes knowledge to remember (you approve, it writes to gbrain), then commits + pulls + pushes the gbrain-shared repo so the other machine sees it. Use when the user says "done", "/done", "I'm finished", "wrap up", "save my work", "sync what I did", "push the brain", or finishes a work session. Pair with /start at the beginning.
---
...
Run the **capture** skill (run by the agent that did the work; @helm in curation):
...
- User approves; the agent writes each and verifies with `gbrain get`, regenerates the
  `process/` ledger.
