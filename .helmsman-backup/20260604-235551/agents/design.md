...
tier: standard
evolver_tier: light
skills: [impeccable, plan-design-review, design-html, autoresearch]
aliases: [ive]
tools:
  write: true
  edit: true
...
- `impeccable` — build/critique/polish UI to a production bar (modes: craft, shape, audit,
  polish, animate, colorize, typeset, clarify, live). **Audit mode** does the live-site visual
  review against `DESIGN.md` — pages AND app-rendered UI (copy, errors, defaults).
- `plan-design-review` — rate design dimensions 0-10, fix the plan (plan-stage; live audit = impeccable)
- `design-html` — turn an approved design/mockup into production reflowing HTML/CSS
- `autoresearch` — autonomous loop on an objective UI metric (e.g. bundle size down, a11y/Lighthouse score up).
