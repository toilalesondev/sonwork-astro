...
### 7.5 CAPTURE the issue-tracker + triage labels (for the ticket skills)
The ticket/issue skills (`to-prd`, `to-issues`, `triage`) need to know where work is tracked —
record it into the one memory (AGENTS.md), don't make them ask every time:
- **Issue tracker** — detect from `git remote -v`: GitHub (→ `gh` CLI) / GitLab (→ `glab`) /
  none → **local markdown** under `process/backlog/` (our default — fits the harness, no external dep).
  Confirm with the user; "other" (Jira/Linear) → record their one-paragraph workflow as prose.
- **Triage labels** — the five canonical roles `needs-triage · needs-info · ready-for-agent ·
  ready-for-human · wontfix`. Default each label = its name; ask only if their tracker already
  uses different strings (map them so skills don't create duplicates).
- **Write** a short `## Issue tracker` block into the per-project AGENTS.md (tracker + label map).
  In gbrain mode it's recallable like the rest of AGENTS.md; in standalone it lives in the file.
  This is what `to-prd`/`to-issues`/`triage` read — one memory, no separate config skill.

### 8. VALIDATE + REPORT
Confirm: process/ scaffolded, all-context.md has real content, source registered (or
"no gbrain — local-only"), AGENTS.md written/left (incl. the issue-tracker block). Report as outcomes:
...
Prove the project is actually onboarded (local memory filled with REAL content):
```
node .helmsman/skills/onboard-project/scripts/validate-onboard.mjs <project-root>
```
Fails if `process/context/all-context.md` still has placeholder text or is near-empty.
Onboarding isn't done until this is green.
