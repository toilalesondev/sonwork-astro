...
// The report is the markdown the write-back agent produces: a numbered list of proposed writes, then
...
// Post-write: each written item should show read-back confirmation. Mode-aware:
//   gbrain mode  → writes look like `gbrain put`, read-back like `gbrain get`.
//   standalone   → writes touch `process/...` files, read-back is a file re-read.
const written =
  (body.match(/\bgbrain put\b/gi) || []).length +
  (body.match(/\bprocess\/(context|agent-memory|takes)\//gi) || []).length;
const confirmed = (body.match(/\b(gbrain get|read.?back|re-?read|confirmed|verified)\b/gi) || []).length;
if (written > 0 && confirmed === 0)
  problems.push("writes present but no read-back confirmation — verify each (gbrain: `gbrain get`; standalone: re-read the file) (Rule 1)");
