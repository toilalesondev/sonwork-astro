...
// --- 14. catalog sync: skills/SKILLS.md matches a fresh regen (WARN) ---
...
// --- 15. dangling skill references: a SKILL.md must not point at a skill we don't have (WARN) ---
// Enforces "every skill links to real things in the harness". Allowlist tool/repo/doc names
// (not skills) that legitimately appear in backticks.
// Known non-skill backtick tokens (tools, repos, docs, CSS, code-tools, label values, verbs).
const REF_ALLOWLIST = new Set([
  "gbrain", "gbrain-shared", "hbrowse", "playwright", "skill-standard", "operating-rules",
  "agents", "harness", "chromium", "setup-matt-pocock-skills",
]);
{
  const realSkills = new Set(dirsIn(skillsDir));
  // Only flag a token if it's referenced in a way that IMPLIES it's a skill: it appears next to
  // the word "skill", or is invoked via the load-path. This avoids CSS/labels/verbs false-positives.
  for (const s of dirsIn(skillsDir)) {
    const body = readFileSync(join(skillsDir, s, "SKILL.md"), "utf8");
    // patterns: "`foo` skill", "skill `foo`", ".helmsman/skills/foo", "the `foo` skill"
    const skillish = new Set();
    for (const m of body.matchAll(/`([a-z][a-z0-9-]{2,30})`\s+skill\b/gi)) skillish.add(m[1]);
    for (const m of body.matchAll(/\bskill\s+`([a-z][a-z0-9-]{2,30})`/gi)) skillish.add(m[1]);
    // only a real load-path counts as a skill reference (avoids "skills/plan/rules" prose)
    for (const m of body.matchAll(/(?:\.helmsman\/)?skills\/([a-z][a-z0-9-]{2,30})\/(?:SKILL\.md|scripts)/gi)) skillish.add(m[1]);
    for (const r of skillish) {
      if (realSkills.has(r) || REF_ALLOWLIST.has(r)) continue;
      warn(`skill "${s}": references skill \`${r}\` which doesn't exist (dangling ref?). Fix or allowlist if it's a tool/repo/doc.`);
    }
  }
}

// --- 16. memory-link: a memory-touching skill should reference the one-memory path (WARN) ---
// Encodes "all skills link with the one memory". Conservative: only flags skills whose body
// mentions memory verbs but NOT a memory anchor.
{
  const memoryVerbs = /\b(recall|write-back|writeback|remember|store the|persist|memory tier|agent-memory|gbrain (put|get|sources|takes))\b/i;
  const memoryAnchor = /\.helmsman\/backend|process\/(context|takes|backlog|active)|agent-memory|gbrain|one memory|AGENTS\.md|harness\/memory/i;
  for (const s of dirsIn(skillsDir)) {
    const body = readFileSync(join(skillsDir, s, "SKILL.md"), "utf8");
    if (memoryVerbs.test(body) && !memoryAnchor.test(body))
      warn(`skill "${s}": touches memory but doesn't reference the one-memory path (.helmsman/backend / process/ / gbrain / AGENTS.md). Link it.`);
  }
}

// --- 17. section presence: the two harness-compliance sections must exist (WARN, conservative) ---
// skill-standard wants Trigger/Procedure/Operating-Rules/Evidence, but Procedure has many valid
// shapes (Config/Loop rules/Output location/Phases/Modes/Steps). We only HARD-care about the two
// that enforce harness behavior: an Operating-Rules bind + an Evidence/Validate proof obligation.
{
  const need = [
    { name: "Operating Rules", re: /^##\s+Operating Rules/mi },
    { name: "Evidence/Validate", re: /^##\s+(Evidence|Validate)/mi },
  ];
  // Rituals / loops / meta-skills legitimately vary from the work-skill shape.
  const SECTION_ALLOWLIST = new Set([
    "start", "done", "refine", "xia", "phase-programs", "adherence-check",
    "brain-sync", "gbrain-upgrade", "autoresearch",
  ]);
  for (const s of dirsIn(skillsDir)) {
    if (SECTION_ALLOWLIST.has(s)) continue;
    const body = readFileSync(join(skillsDir, s, "SKILL.md"), "utf8");
    for (const sec of need) if (!sec.re.test(body)) warn(`skill "${s}": missing a "${sec.name}" section (skill-standard compliance).`);
  }
}
