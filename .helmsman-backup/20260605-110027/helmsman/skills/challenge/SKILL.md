...
The GO/CAUTION/STOP verdict + the edge-case/scenario table, and (on high-risk work) the
`risk-gate.json` written. A STOP must name the specific failure modes that block BUILD — never a
verdict without the table behind it (Rule 1).
