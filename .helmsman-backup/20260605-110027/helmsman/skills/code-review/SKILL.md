...
The verdict (GO/CAUTION/STOP) with each finding as `file:line` + the concrete failure mode
(input → bad outcome); high-risk emits `review-decision.json`. Zero findings is valid — report
APPROVE, never manufacture nits. Never claim reviewed without the cited findings (Rule 1).
