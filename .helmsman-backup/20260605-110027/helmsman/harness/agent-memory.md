...
> **Evolver tier (`light`).** Writing/clustering agent-memory landmines is an *evolver*
> operation — flat across model tiers (see `model-routing.md` role axis). Recall + write +
> graduation run at `light`, not the agent's solve tier.
