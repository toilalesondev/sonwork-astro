...
**Carve-out (safety) — DECIDED, not pending:** Loop B (refine — edits an agent's *durable*
instructions) and Loop D (risk-evidence — *gates* auth/RLS/billing safety) **stay at the
agent's normal tier.** This is a settled decision, evidence-backed: the harness is
*underconfident, not over-betting* (calibration: 5/5 resolved correct, Brier 0.06), so there
is no cost pressure to cheapen its highest-stakes loops. The flatness finding covers routine
delta-production, NOT durable self-modification or safety gating. (A future option — B/D
*draft* at `light` behind the existing human approval gate — is explicitly out of scope
unless cost data later says otherwise.)
