...
> **PREFER THE WARM MCP for recall/query.** The `gbrain` CLI pays a ~4s cold-start every call
> (fresh process + Supabase connect + RLS) → ~11s/query. The **MCP server (`gbrain serve`,
> already wired in opencode/Claude Code) is a warm process** — it pays that connect ONCE, so
> in-session recall is far faster. **In an MCP host, use the `gbrain_*_ide` tools for
> recall/query; fall back to the `gbrain` CLI only when no MCP host is present** (e.g. a bare
> script). Deterministic scripts (e.g. `calibrate/score-takes`) keep the CLI — they're not in
> the hot recall path. (Tier-1 local `process/context/` recall is instant regardless — read it first.)

| Verb | Preferred (warm MCP) | CLI fallback (cold) |
|------|----------------------|---------------------|
| `recall` | MCP `gbrain_query_ide` (pass `source_id`) | `gbrain query "..." --source <id>` |
| `remember` | MCP `gbrain_put_page_ide` → verify `gbrain_get_page_ide` | `gbrain put <slug>` → verify `gbrain get <slug>` |
| `recall_agent_memory` | MCP `gbrain_query_ide` "agent-memory/<agent>/<topic>" | `gbrain query "agent-memory/<agent>/<topic>" --source <project>` |
| `remember_agent_memory` | MCP `gbrain_put_page_ide` | `gbrain put "agent-memory/<agent>/<topic>" --source <project>` |
| `write_back` | numbered MCP puts/facts/takes, user-approved, each verified by read-back | same via CLI |
