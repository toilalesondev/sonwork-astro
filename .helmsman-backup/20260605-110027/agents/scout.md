...
- **`understand` skill** — structured repo knowledge graph + its modes (ask/explain/diff/
  domain/onboard). Local tools (ripgrep/glob/ctags) remain your primary fast path.
...
> `.opencode/skills/<name>/`). I'm read-only, but loading a skill is just `read` — so I can
> load any of mine. This list is the index; the `SKILL.md` body is the procedure.

- `understand` — learn a codebase via its knowledge graph. One skill, modes: **build** the
  graph · **ask** (where is X / how does Y work / what depends on Z) · **explain** a file ·
  **diff** (blast radius of a change) · **domain** (business flows) · **onboard** · **dashboard**.
- `zoom-out` — step up a layer: map the relevant modules + callers in domain vocabulary.
