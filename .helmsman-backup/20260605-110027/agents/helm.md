...
tier: standard
evolver_tier: light
skills: [setup-gbrain, brain-sync, gbrain-upgrade, gstack-upgrade, health, privatize, skill-stocktake, memory-check, context, onboard-project, evolve-memory, harness-audit, capture, calibrate, adherence-check]
aliases: []
tools:
  write: true
  edit: true
...
- `setup-gbrain` — install + wire the (optional) gbrain memory tier for this machine.
- `brain-sync` — keep the shared gbrain repo + DB in step across machines.
- `gbrain-upgrade` — upgrade the gbrain binary/tooling. `gstack-upgrade` — upgrade the (optional) external gstack tool when present.
- `onboard-project` — scaffold a project into the harness (first-touch setup).
- `privatize` — turn ANY external skill repo into native Helmsman skills (study → extract logic → de-bloat → wire). Absorbs the old find-skills/write-a-skill/skillify.
- `skill-stocktake` — audit the whole skill set for quality/scope-fit/overlap; verdicts Keep/Improve/Update/Retire/Merge/Split. The governance behind the Separation Law.
- `memory-check` — 360° health test of the memory + learning layer (recording integrity, query efficiency, recall-precision, loop-yield, flywheel; both backends). Run after any memory/skill/loop change.
- `harness-audit` — verify harness coherence (MUST be GREEN after any harness change).
- `adherence-check` — measure Harness-Following-Rate; feeds Loop B refine.
- `capture` — write-back (Loop A): propose + verify memory writes after work.
- `evolve-memory` — cluster landmines; graduate a recurring pattern to a skill (Loop C).
- `calibrate` — score resolved takes (accuracy + Brier); write the calibration profile (Loop E).
- `health` — code/system health dashboard.
- `context` — save/restore working state across sessions + handoffs.
