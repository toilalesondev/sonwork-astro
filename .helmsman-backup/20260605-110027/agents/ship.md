...
tier: standard
evolver_tier: light
skills: [ship, land-and-deploy, code-review, claude, qa, browse, scrape, investigate, canary, benchmark, benchmark-models, health, retro, document-release, landing-report, verify]
aliases: [gstack]
tools:
  write: true
  edit: true
...
**Verify/review**
- `verify` — diff-aware test run (changed files → affected tests; escalate on high-fan-out).
- `code-review` — production-readiness review (edge-case scout first, then the backend-aware checklist).
- `qa` — QA a web app; **fix mode** tests AND fixes, **report mode** finds bugs without fixing.
- `investigate` — root-cause a failing / flaky / "was working yesterday" run before touching code.
- `claude` — second-opinion review/challenge from an outside model on a risky diff.

**Browser**
- `browse` — drive a headless browser to verify a deploy / dogfood a flow / capture evidence.
- `scrape` — pull data off a page (read-only).

**Ship**
- `ship` — bump VERSION, changelog, commit, push, open the PR.
- `land-and-deploy` — merge the PR, wait on CI/deploy, verify prod health.
- `landing-report` — read-only queue dashboard (which VERSION slots are claimed).

**Monitor**
- `canary` — watch live prod post-deploy for errors/regressions vs baseline.
- `benchmark` — page-speed / web-vitals regression vs baseline. `benchmark-models` — cross-model comparison.
- `health` — code-quality dashboard. `retro` — engineering retrospective.
