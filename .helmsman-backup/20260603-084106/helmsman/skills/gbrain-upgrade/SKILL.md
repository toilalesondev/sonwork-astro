...
| `federated_read` (a super-reader client = all sources) | `oauth_clients` table | ✅ yes |
...
| chat-provider gateway (if any) | host config (NOT a gbrain patch) | ✅ yes |
...
```
(Remote MCP clients reconnect automatically — same endpoint/port/auth.)
...
- **`sync.repo_path` is one global, last-writer-wins.** A bare `gbrain sync <dir>` flips it. Keep it at your configured default-source repo path. (PR #1771 makes write-through ignore it per-source — once merged, this matters less.)
