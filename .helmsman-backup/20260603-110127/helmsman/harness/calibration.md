...
Periodic (e.g. weekly/monthly) or on demand ("how calibrated are we?"). Owned by `@brain`
(memory) with `@data` (analysis). It's **read-mostly**: it scores + reports + writes a
calibration *profile* back to gbrain; it does not rewrite the original takes.
