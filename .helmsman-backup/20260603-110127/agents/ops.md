...
description: >-
  ...
  analyzing metrics (@data), NOT for your personal comms/triage (@chief). Owns business ops.
mode: subagent
model: sonnet
tier: standard
...
- Analyzing the numbers (revenue trends, cohort analysis) → `@data`.
...
- **End:** propose ops decisions/SOPs to `@brain` for write-back (both tiers).
...
Numbers → `@data`. Your time/comms → `@chief`.
