...
### 2. TRIAGE — one verdict per incoming skill (apply the Separation Law)
Default is **SEPARATE** (one skill = one narrow concern — see `harness/skill-standard.md`).
Judge scope-fit: *can an agent fully utilize this when it needs it, without loading concerns it
doesn't?*
- **KEEP** — real, distinct concern an agent needs → privatize as its OWN skill.
- **MERGE → X** — ONLY when *trivially identical* to X (same procedure + a flag/direction, e.g.
  qa report-vs-fix). **Different perspective is NOT a merge** — it's a separate skill. When
  unsure, do NOT merge.
...
Record the verdict table — it's the report-after artifact. (Post-ingest, `skill-stocktake`
re-audits the whole set for scope-fit/overlap.)
