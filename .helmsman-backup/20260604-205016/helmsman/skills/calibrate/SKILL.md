...
**gbrain mode:**
```
gbrain takes scorecard [<holder>] [--domain <prefix>]   # count, accuracy, Brier
gbrain takes <slug>                                      # the rows on a page
```
Bets are recorded with **`gbrain takes add <slug> --claim "..." --kind bet --who <holder>
--weight N`** and resolved with **`gbrain takes resolve <slug> --row N --quality
correct|incorrect|partial`**. ⚠️ Use these commands — do NOT hand-write a `## Takes` fence via
`gbrain put`; a hand-written fence only parses during a full `gbrain cycle`, so `takes add`/
`resolve` is the path that lands in the DB immediately. Scope with `--domain`/`<holder>`.
