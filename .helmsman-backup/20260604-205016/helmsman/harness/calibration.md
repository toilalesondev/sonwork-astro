...
## Two modes (Loop E works in BOTH)

- **gbrain mode:** takes are typed/weighted gbrain rows; score with `gbrain takes_scorecard`
  / `takes_calibration`.
- **standalone mode:** takes live in an append-only file ledger `process/takes/<holder>.jsonl`
  (`{id,holder,kind,claim,weight,since,resolved,outcome}`); score with
  `node skills/calibrate/scripts/score-takes.mjs process/takes`. Same metrics (accuracy +
  Brier), no gbrain. Bets are written during write-back (Loop A) and resolved when reality
  decides them. This is why Loop E is fully available standalone — not gbrain-gated.
