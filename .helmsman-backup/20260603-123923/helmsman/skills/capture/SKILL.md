...
name: capture
version: 1.0.0
description: gbrain-native write-back (Loop A). The agent that did the work (or @helm in a curation pass) analyzes the session and proposes a numbered list of gbrain writes (pages/facts/takes) plus plan archival, backlog capture, and agent-memory updates. User approves; the agent writes and verifies each with gbrain get, then regenerates the process/ ledger. Memory is a shared capability — see harness/memory.md.
---
...
curation passes. Runs after meaningful work. This
...
1. **Pages** — `gbrain put <slug>` for durable knowledge. Slug convention:
   `projects/<name>/<area>/<topic>` (e.g. `projects/acme/web/share-modal`).
...
gbrain is semantic, graph-linked, and cross-machine. Knowledge written from one project
is recallable from another project and any machine sharing the brain. brv was
project-local and duplicated this at extra token cost — it is retired from the flow.
...
```
Checks: numbered proposal (approvable item-by-item) + each write has `gbrain get`
read-back confirmation (Rule 1). Fix and re-run until green.
