...
I'm **read-only** — I PRODUCE the understanding and **hand it to `@brain` to land** in
...
- **Local (Tier 1):** the content for `process/context/all-context.md` (+ a domain pack
  once a domain has 3+ durable docs — see `context-routing.md`). @brain writes it.
- **Durable (Tier 2):** `projects/<project>/repo-map` + `agent-memory/scout/<topic>`
  landmines → @brain `write_back`s to the project's gbrain source, verifies by read-back.
...
citations from the explore, and @brain's read-back confirming the understanding landed.
...
- **End:** refresh local `process/context/` AND propose durable architecture facts to
  `@brain` for `write_back` (both tiers) — so the next explore starts smarter. No brv.
