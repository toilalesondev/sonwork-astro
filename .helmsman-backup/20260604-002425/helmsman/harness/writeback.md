...
**Owner: the agent that did the work** (it has the context — no lossy hand-off to a memory
agent). Memory is a shared capability, not an agent (see `memory.md`). Runs after a
non-trivial BUILD/SHIP — the "harness learns" mechanic. Writes to **both** memory tiers:
the **local** domain pack (`process/context/<domain>/`) for instant in-session reuse, AND
**gbrain** for durable, cross-project knowledge. Local keeps the agent fast; gbrain makes
it compound. Cross-cutting curation (dedup, slug hygiene, cross-repo synthesis) is `@helm`'s
periodic job, not part of every write-back.

> A read-only agent (`@scout`, `@chief`'s data work) PRODUCES the write-back proposal and
> hands the content to a write-capable agent (or `@helm`) to land it — per `handoff.md`.

**Run write-back at the `evolver` tier (`light`).** This is an *evolver* operation —
producing harness deltas — which is flat across model tiers (see `model-routing.md` role
axis). The expensive tier was for the *solve*; the write-back that follows drops to `light`.
