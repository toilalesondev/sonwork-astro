...
- `qa` — full QA loop that tests AND fixes. `qa-only` — report bugs, never fix (report-only).
...
**Monitor**
- `canary` — watch live prod post-deploy for errors/regressions.
- `benchmark` — page-performance/web-vitals regression check. `benchmark-models` — cross-model skill comparison.
- `health` — code-quality dashboard. `retro` — weekly engineering retrospective.
