...
// Vendored skills whose repo copy intentionally diverges from upstream gstack
// (e.g. a deliberate trim). Drift on these is expected — stays quiet.
const VENDOR_DRIFT_ALLOWLIST = new Set(["qa"]);
// Where the machine-global host skill mirrors live (for divergence checks).
const GLOBAL_MIRRORS = [
  join(homedir(), ".claude", "skills"),
  join(homedir(), ".config", "opencode", "skills"),
];
// Upstream gstack checkout root (override with GSTACK_HOME). Skipped if absent.
...
const ourSkills = existsSync(skillsDir) ? readdirSync(skillsDir) : [];
const vendorSkills = existsSync(vendorDir) ? readdirSync(vendorDir) : [];
...
// --- 9. VENDORED skills: repo copy vs upstream gstack (WARN; allowlisted trims are quiet) ---
if (existsSync(GSTACK_HOME)) {
  const upBases = [
    join(GSTACK_HOME, ".opencode", "skills"),
    join(GSTACK_HOME, ".claude", "skills"),
    GSTACK_HOME,
  ].filter(existsSync);
  for (const s of vendorSkills) {
    if (VENDOR_DRIFT_ALLOWLIST.has(s)) continue;
    const repoHash = sha(join(vendorDir, s, "SKILL.md"));
    if (!repoHash) continue;
    let upPath = null;
    for (const b of upBases) {
      for (const cand of [join(b, `gstack-${s}`, "SKILL.md"), join(b, s, "SKILL.md")]) {
        if (existsSync(cand)) { upPath = cand; break; }
      }
      if (upPath) break;
    }
    if (!upPath) continue; // not present upstream → can't compare; not drift
    if (sha(upPath) !== repoHash) warn(`vendor skill "${s}" drifts from upstream gstack (${upPath}) — re-vendor or allowlist if intentional`);
  }
}

// --- 10. load-path: every agent documents HOW to load a skill (HARD fail) ---
