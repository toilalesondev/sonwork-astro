...
**Re-anchor here** (see `adherence.md`): adherence decays over a long build, so at this
check-in also re-state — in ≤5 lines — the remaining **Verification Evidence** items + any
**loaded-skill hard requirements** + the **risk-gate status** (if high-risk). Landmines
only, not the whole plan. This is a cheap `evolver`-tier re-statement, not a new gate.
