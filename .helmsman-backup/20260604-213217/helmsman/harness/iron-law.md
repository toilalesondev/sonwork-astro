...
- **Explore** → routes to `@scout` (this repo) or `@helm` (cross-repo / curation)
...
- **Remember / write back** → every agent does it (shared capability); cross-cutting curation → `@helm`
...
## Handoffs go through the orchestrator (the relay)

Because specialists are isolated context windows, they **cannot hand to each other
directly** — they return to the orchestrator with a status, and it dispatches the next
agent. The durable **artifact** (the plan, `verification.json`, a findings file) is the
handoff packet; the chat is not. "Hand back to the builder" = "return to orchestrator →
orchestrator re-dispatches." Full protocol: `harness/handoff.md`.
