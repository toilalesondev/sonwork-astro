...
## Honest limit — how strong the auto re-anchor can be (per host)

The agent re-states obligations as discipline; the hooks *nudge* it. What the hook can do is
bounded by the host's plugin API:
- **opencode:** the plugin fires a **toast + log** every N tool calls (`hooks/opencode/
  helmsman.js`). It canNOT inject text into the model's context mid-run — the only
  context-injection point is `experimental.session.compacting` (at compaction). So mid-run
  is a *reminder*, not a forced re-injection; the deeper re-injection happens at compaction.
- **Claude Code:** `session-hydrate` injects at SessionStart / after compaction (stdout
  becomes context) — also not a per-step mid-run injection.
This is an honest ceiling, not a gap to "fix": neither host exposes a forced mid-trajectory
context push today. The re-anchor is therefore: agent discipline (primary) + a host nudge
(toast/log) + full re-injection at compaction. If a host later exposes a mid-run context
hook, wire it here.
