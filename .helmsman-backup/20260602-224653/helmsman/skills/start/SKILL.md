---
name: start
description: Begin-of-session ritual for a shared cross-machine brain. Pulls the gbrain-shared brain repo, reconciles it into the DB, then restores your last working context so you pick up exactly where you left off. Use when the user says "start", "/start", "start my session", "begin", "sync the brain first", "what was I doing", or sits down to work on a machine that shares the Supabase brain. Pair with /done at the end.
---
...
```
This does `git pull --ff-only` on the brain repo (`~/gbrain-shared` or `/srv/gbrain-shared`)
then `gbrain sync --source default` to reconcile files → DB. Idempotent.

If `brain-sync.sh` isn't on this machine, fall back to:
```bash
cd ~/gbrain-shared && git pull --ff-only && gbrain sync --source default
```
