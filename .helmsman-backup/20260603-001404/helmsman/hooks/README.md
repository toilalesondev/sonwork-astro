...
| **session-hydrate** | re-inject the Helmsman contract + in-flight `process/active/` plans so the agent doesn't forget the rules after compaction / on session start | `experimental.session.compacting` pushes context | `SessionStart` prints context |
