...
## Intent → agent (the 11-agent map)

| Intent (keywords) | Phase | Route to |
|-------------------|-------|----------|
| idea, "what if", "should we", brainstorm, worth building, PRD | IDEATE | `@product` |
| understand, how does X work, where is, explain, what changed | EXPLORE | `@scout` (this repo) / `@helm` (cross-repo, code graph, curation) |
...
| remember, save this, what do we know about, recall | MEMORY | **shared capability** — every agent recalls + writes back; cross-cutting curation/cross-repo → `@helm` |
...
| triage my asks, draft email/message, summarize, what to focus on, **metrics/analytics, market/competitor research, "what does the data say"** | CHIEF | `@chief` |
...
7. **Feature request** → domain builder by surface (UI→@design, logic→@eng, data→@backend,
   ...
   @chief (your time/comms + numbers/analysis).
...
- **@chief vs @scout** — @chief analyzes *numbers/market*; @scout explores *the codebase*.
...
ideation→product, ship/QA→ship), `@evera`→`@scout`, `@tan`→ memory is now a shared capability (curation → `@helm`), `@andy`/`@data`→`@chief`.
