...
version: 1.0.0
description: >-
  Run the full plan-review gauntlet automatically — all relevant lenses (ceo, eng, design,
...
Orchestrates `plan-review` across every relevant lens, auto-deciding the clear calls and
...
### 1. Run the lenses in sequence
Run `plan-review` for each relevant lens in order — typically **ceo → eng → design → devex**
(skip lenses that don't apply, e.g. no UI → skip design). Each lens rates dimensions + proposes fixes.
