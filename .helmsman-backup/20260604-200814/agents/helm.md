...
tier: standard
evolver_tier: light
skills: [setup-gbrain, brain-sync, gbrain-upgrade, gstack-upgrade, health, privatize, context, onboard-project, evolve-memory, harness-audit, capture, calibrate, adherence-check]
aliases: []
tools:
  write: true
  edit: true
...
- `privatize` — turn ANY external skill repo into native Helmsman skills (study → extract logic → de-bloat → merge → wire). Absorbs the old find-skills/write-a-skill/skillify.
