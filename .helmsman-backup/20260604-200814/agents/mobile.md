...
tier: standard
evolver_tier: light
skills: [ios-qa, ios-bridge, generate-plan, challenge, verify, code-review, autoresearch]
aliases: []
tools:
  write: true
  edit: true
...
- `ios-qa` — live-device QA loop via the StateServer. Modes: **qa** (find bugs), **fix**
  (on-device patch + re-verify), **design** (visual audit vs HIG + DESIGN.md). Dormant without a device.
